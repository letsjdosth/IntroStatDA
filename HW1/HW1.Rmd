...
First, let me define the euclidean norm function.
```{r}
...
'hp' has an outlier (about 350.)
However, because of different scales, the boxplots except for 'disp' and 'hp' are not plotted well.
So, let me show another parallel boxplots for 7 covariates.
```{r}
...
Next, make variable for ratios.
Actually, we can find rows having the maximum ratio value and the minimum ratio value.
```{r}
...
However, following the problem's direction, let's sort the data frame by ratio.
```{r}
mammals$ratio <- r
...
If we use some transformation, it will be better. But I'll skip it.
...
Moreover, when drawing the 3-D plot, samples from the 'runif' in R are better than the samples of 'randu' data frame,
...
Let's repeat the above code block with 10-column matrix.
```{r}
...
```{r}
pairs(Boston[,-1],  upper.panel = panel.cor, panel = panel.smooth,
      cex = 1.5, pch = 1, bg = "light blue", horOdd=TRUE,
      diag.panel = panel.hist, cex.labels = 2, font.labels = 2)
```
...
```
In the case of crime rate, there are so many data points outside of whiskers.
We can say thet there are many suburb with particularly high crime rates.
...
For other two variables, there is no sample that has a particularly high value.
...
For example, its zn and medv values are lowest, and age, rad and black values are highest.
Not only that, dis, tax, ptratio, and lstat values are close to the maximum or minimum value of each predictor.
...
```
They have low per capita crime rate, and nox values.
In addition, they indicates high black, lstat, medv values.